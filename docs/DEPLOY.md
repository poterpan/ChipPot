...
1. **設定**頁：
   ...
   - 三段**通知模板**（逾期催繳／開繳通知／常駐繳費訊息）——有即時預覽與格式檢查（支援 Discord 風 markdown）。
   - （選填）**繳費通知**：填 Bark 推播網址或 Webhook 網址（Discord／Google Chat／Slack 皆可，依網址自動判斷格式），有人繳費就推播給你，並附「點一下直接開該筆審核」的深連結（連結用 `ADMIN_ORIGIN`，即你的後台網域，免額外設定）。
