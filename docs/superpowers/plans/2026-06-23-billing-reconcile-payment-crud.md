...
const TS = "2026-05-01T00:00:00.000Z";
const WS = 9200, SUB = 9200;
const router = buildAdminRouter();
// Access 已在外層；測試直接打 router，帶 workspace ctx header（沿用既有測試慣例）。
function call(method: string, path: string, body?: unknown) {
  return router.handle(
    new Request(`https://x${path}`, { method, headers: { "content-type": "application/json", "x-test-workspace": String(WS) }, body: body ? JSON.stringify(body) : undefined }),
...
> 注意：`call()` 的 workspace 注入方式須對齊既有 route 測試（檢查 `test/routes/` 既有檔如何提供 `wsId` ctx；若用 header 以外機制，照抄那個 helper）。
...
> audit 資料表名以實際 schema 為準（既有為 `audit_logs`；若不同請對齊）。
...
describe("POST /admin/billing/:period/sync", () => {
  it("dry_run returns diff without writing", async () => {
    // active sub with no bill in an opened period
    await env.DB.batch([
      env.DB.prepare(`INSERT INTO notification_logs (workspace_id,type,period,plan_id,user_id,subscription_id,sent_at) VALUES (?,?,?,?,?,?,?)`).bind(WS,"billing_opened","2027-09",0,0,0,TS),
    ]);
    const res = await call("POST", "/admin/billing/2027-09/sync", { dry_run: true });
    expect(res.status).toBe(200);
    const d = await res.json() as any;
    expect(d.opened).toBe(true);
    expect(d.add.length).toBe(1); // SUB has no 2027-09 bill yet
    const cnt = await env.DB.prepare("SELECT COUNT(*) c FROM payments WHERE workspace_id=? AND period='2027-09'").bind(WS).first<{c:number}>();
    expect(cnt?.c).toBe(0);
  });
  it("apply writes bills and returns counts", async () => {
    const res = await call("POST", "/admin/billing/2027-09/sync", { dry_run: false });
    const r = await res.json() as any;
    expect(r.ok).toBe(true);
    expect(r.applied.added).toBe(1);
    const cnt = await env.DB.prepare("SELECT COUNT(*) c FROM payments WHERE workspace_id=? AND period='2027-09'").bind(WS).first<{c:number}>();
    expect(cnt?.c).toBe(1);
  });
